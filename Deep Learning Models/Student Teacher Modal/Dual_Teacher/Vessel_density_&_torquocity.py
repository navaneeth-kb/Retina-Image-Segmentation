...
history1=teacher_model1.fit(x_train, y_train, epochs=x, batch_size=8, validation_split=0.1)
history2=teacher_model2.fit(x_train, y_train, epochs=x, batch_size=8, validation_split=0.1)
...
def evaluate_models(teacher_model1, teacher_model2, student_models, x_data, y_data):
    ...
    overall_avg_accuracy = (avg_teacher_accuracy + avg_student_accuracy) / (num_students)
    overall_avg_mean_iou = (avg_teacher_mean_iou + avg_student_mean_iou) / (num_students)
    overall_avg_auc = (avg_teacher_auc + avg_student_auc) / (num_students)
    overall_avg_f1_score = (avg_teacher_f1 + avg_student_f1_score) / (num_students)
...
def calculate_vessel_density(mask):
    """
    Compute vessel density as the percentage of vessel pixels.
    """
    vessel_pixels = np.sum(mask > 0)
    total_pixels = mask.size
    return (vessel_pixels / total_pixels) * 100  # Percentage
...
def calculate_tortuosity(mask):
    ...
    # Ensure binary mask (convert grayscale to binary if needed)
    binary_mask = (mask > 0).astype(np.uint8)
    
    # Skeletonize the vessel structure
    skeleton = skeletonize(binary_mask)

    # Compute total vessel length
    vessel_length = np.sum(skeleton)

    # Label skeleton to extract individual vessels
    labeled_skeleton = label(skeleton)

    # Get the longest vessel segment
    max_length = 0
    max_region = None
    ...
    if max_region is not None:
        ...
        # Compute tortuosity
        tortuosity = vessel_length / euclidean_distance if euclidean_distance > 0 else 0
        return tortuosity
    else:
        return 0  # If no vessels detected, return 0

def predict_masks_in_folder(student_models, folder_path):
    """
    Predict masks and compute vessel density, fractal dimension, and tortuosity.
    """
    ...
    for image_file in image_files:
        ...
        vessel_density = calculate_vessel_density(predicted_mask)
        vessel_densities_test.append(vessel_density)
        ...
        tortuosity = calculate_tortuosity(predicted_mask)
        tortuosities_test.append(tortuosity)
        print(f"Tortuosity: {tortuosity:.4f}")

    # Compute averages
    avg_pred_density = np.mean(vessel_densities_test)
    avg_tortuosity = np.mean(tortuosities_test)
    print(f"Average Vessel Density (Predicted): {avg_pred_density:.2f}%")
    print(f"Average Tortuosity: {avg_tortuosity:.4f}")

    return vessel_densities_test, fractal_dimensions_test, tortuosities_test

# Compute vessel density for ground truth masks
densities = [calculate_vessel_density(mask) for mask in y_train]  
average_density = np.mean(densities)
print(f"Average Vessel Density (Ground Truth): {average_density:.2f}%")
