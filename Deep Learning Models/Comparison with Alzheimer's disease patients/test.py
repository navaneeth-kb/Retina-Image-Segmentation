...
for student_model in student_models:
    ...
    student_model.fit(x_train, [y_train, combined_teacher_preds], epochs=1, batch_size=8, validation_split=0.1)
...
def fractal_dimension(Z, threshold=0.9):
    print((Z.shape))
    Z = np.squeeze(Z)  # Removes the last dimension if it is 1

    def boxcount(Z, k):
        # Sum of values in each kxk box
        S = np.add.reduceat(
    ...
    # Largest power of 2 less than or equal to p
    n = 2**np.floor(np.log2(p))

    # Extract exponent
    n = int(np.log2(n))

    # Box sizes from 2^n to 2^1
